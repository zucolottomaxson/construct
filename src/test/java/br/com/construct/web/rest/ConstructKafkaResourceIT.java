...
import br.com.construct.config.KafkaProperties;
...
class ConstructKafkaResourceIT {
    ...
    void setup() {
        ...
        ConstructKafkaResource kafkaResource = new ConstructKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/construct-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/construct-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
